...
def init_db():
    ...
    try:
        ...
        cur.execute("SELECT column_name FROM information_schema.columns WHERE table_name='users' AND column_name='p2p_wallet';")
        if not cur.fetchone():
            cur.execute("ALTER TABLE users ADD COLUMN p2p_wallet REAL DEFAULT 0.0;")
            
        cur.execute("SELECT column_name FROM information_schema.columns WHERE table_name='users' AND column_name='wheel_spun';")
        if not cur.fetchone():
