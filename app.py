...
.msg { color: #10b981; font-size: 13px; margin-bottom: 15px; background: rgba(16,185,129,0.1); padding: 8px; border-radius: 6px; }
.switch-link { color: #a78bfa; font-size: 13px; margin-top: 15px; display: block; text-decoration: none; }
</style>
</head>
<body>
...
<a href="/register" class="switch-link">Don't have an account? Register Here</a>
</div>
</body>
</html>"""

USER_REGISTER_HTML = """<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
...
h2 { color: #10b981; font-size: 20px; margin-top: 15px; font-weight: 800; letter-spacing: 1px; margin-bottom: 25px; }
input { width: 100%; padding: 12px; background: #130620; border: 1px solid #4a256d; border-radius: 8px; color: white; margin-bottom: 18px; box-sizing: border-box; outline:none; font-size:14px; }
input:focus { border-color: #10b981; }
button { width: 100%; padding: 12px; background: linear-gradient(135deg, #10b981, #059669); border: none; font-weight: bold; cursor: pointer; border-radius: 8px; color: white; font-size: 15px; letter-spacing:1px; }
.switch-link { color: #a78bfa; font-size: 13px; margin-top: 15px; display: block; text-decoration: none; }
</style>
</head>
<body>
...
<input type="text" name="referrer" placeholder="Sponsor UID (Optional)" value="{{ ref_code }}">
<button type="submit">JOIN B4U NETWORK</button>
</form>
<a href="/" class="switch-link">Already a member? Login</a>
</div>
</body>
</html>"""
...
.box.p2p { border-top-color: #3b82f6; }
.box.tree { border-top-color: #fdb913; }
...
.btn { color: white; padding: 10px 18px; border: none; border-radius: 8px; cursor: pointer; font-weight: bold; white-space: nowrap; transition: 0.2s; }
...
.ref-box { background: #10041a; border: 1px dashed #fdb913; padding: 12px; border-radius: 8px; color: #fdb913; font-family: monospace; font-size: 13px; display: flex; justify-content: space-between; align-items: center; margin-top: 10px; gap: 10px; flex-wrap: wrap; }
.copy-btn { background: #fdb913; color: #140620; border: none; padding: 6px 14px; font-weight: bold; border-radius: 6px; cursor: pointer; }
.msg-alert { padding: 12px; border-radius: 8px; margin-bottom: 20px; font-size: 14px; font-weight: bold; text-align: center; }
...
{% if msg %}
<div class="msg-alert msg-{{ msg_type }}">{{ msg }}</div>
{% endif %}
...
<p style="font-size: 13px; color: #a78bfa; margin: 0;">Share your exact live domain link to register downline members directly under you:</p>
<div class="ref-box">
<span id="refUrl">{{ ref_url }}</span>
<button onclick="copyRefLink()" class="copy-btn">COPY LINK</button>
</div>
</div>
...
<input type="text" name="recipient_uid" placeholder="Recipient Node UID (e.g. B4U1003)" required style="width:230px;">
<input type="number" step="0.01" name="amount" placeholder="Amount ($)" required>
<button type="submit" class="btn" style="background: linear-gradient(135deg, #3b82f6, #1d4ed8);">TRANSFER FROM PROFIT WALLET</button>
</form>
</div>

<div class="box tree">
<h2>🌳 Downline Network Tree</h2>
...
<tr>
<td><b style="color:#fdb913;">L{{ member.level }}</b></td>
...
{% else %}
<tr>
<td colspan="6" style="text-align:center; color:#a78bfa;">No downline team members found yet. Share your link to recruit partners!</td>
</tr>
{% endfor %}
</tbody>
</table>
...
<tr>
<td><span style="color:#3b82f6; font-weight:bold;">P2P TRANSFER</span></td>
<td>From <code>{{ p2p.sender }}</code> ➔ To <code>{{ p2p.recipient }}</code></td>
<td><b>${{ p2p.amount }}</b></td>
<td><i>{{ p2p.created_at }}</i></td>
</tr>
{% endfor %}
{% for dep in deposits %}
<tr>
...
var refText = document.getElementById("refUrl").innerText;
...
# Exact Live Render Domain Link Generation
base_url = request.host_url.rstrip('/')
if not base_url.startswith('https://') and 'onrender.com' in request.host:
base_url = f"https://{request.host}"

ref_url = f"{base_url}/register?ref={uid}"
...
(uid, amount, method, address, status, datetime.utcnow().strftime("%Y-%m-%d %H:%M"))
)
conn.commit()
