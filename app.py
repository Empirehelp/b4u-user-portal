...
h2 { color: #fdb913; font-size: 20px; margin-top: 15px; font-weight: 800; letter-spacing: 1px; margin-bottom: 25px; }input { width: 100%; padding: 12px; background: #130620; border: 1px solid #4a256d; border-radius: 8px; color: white; margin-bottom: 18px; box-sizing: border-box; outline:none; font-size:14px; }
input:focus { border-color: #fdb913; box-shadow: 0 0 8px rgba(253, 185, 19, 0.3); }button { width: 100%; padding: 12px; background: linear-gradient(135deg, #fdb913, #e28700); border: none; font-weight: bold; cursor: pointer; border-radius: 8px; color: #1a0928; font-size: 15px; letter-spacing:1px; transition: 0.3s; }
button:hover { transform: translateY(-2px); box-shadow: 0 5px 15px rgba(253, 185, 19, 0.4); }.err { color: #ff4d4d; font-size: 13px; margin-bottom: 15px; background: rgba(255,77,77,0.1); padding: 8px; border-radius: 6px; }.msg { color: #10b981; font-size: 13px; margin-bottom: 15px; background: rgba(16,185,129,0.1); padding: 8px; border-radius: 6px; }.switch-link { color: #a78bfa; font-size: 13px; margin-top: 15px; display: block; text-decoration: none; }</style></head><body><div class="login-card">""" + BIG_COIN_SVG + """<h2>B4U MEMBER PORTAL</h2>{% if error %}<div class="err">{{ error }}</div>{% endif %}{% if msg %}<div class="msg">{{ msg }}</div>{% endif %}<form action="/login" method="POST"><input type="text" name="uid" placeholder="Node UID (e.g. B4U1001)" required><input type="password" name="password" placeholder="Access Key" required><button type="submit">LOGIN TO NETWORK</button></form><a href="/register" class="switch-link">Don't have an account? Register Here</a></div></body></html>"""
...
.login-card { background: #2b1442; border-top: 5px solid #10b981; padding: 40px 35px; border-radius: 16px; width: 340px; box-shadow: 0 15px 35px rgba(0,0,0,0.6); text-align: center; }
h2 { color: #10b981; font-size: 20px; margin-top: 15px; font-weight: 800; letter-spacing: 1px; margin-bottom: 25px; }input { width: 100%; padding: 12px; background: #130620; border: 1px solid #4a256d; border-radius: 8px; color: white; margin-bottom: 18px; box-sizing: border-box; outline:none; font-size:14px; }
input:focus { border-color: #10b981; }button { width: 100%; padding: 12px; background: linear-gradient(135deg, #10b981, #059669); border: none; font-weight: bold; cursor: pointer; border-radius: 8px; color: white; font-size: 15px; letter-spacing:1px; }
...
.brand-head { display: flex; align-items: center; gap: 12px; }.brand-title { font-weight: 800; letter-spacing: 1.5px; color: #ffffff; }.trading-hero { background: linear-gradient(135deg, #2b1442 0%, #1c0b2e 100%); border: 2px solid #fdb913; border-radius: 16px; padding: 22px 28px; margin-bottom: 25px; box-shadow: 0 8px 30px rgba(253, 185, 19, 0.15); display: flex; flex-wrap: wrap; justify-content: space-between; align-items: center; gap: 20px; }
.hero-left { display: flex; align-items: center; gap: 22px; }.coin-details-title { font-size: 22px; font-weight: 900; color: #ffffff; letter-spacing: 1px; display: flex; align-items: center; gap: 10px; }.pair-badge { background: #fdb913; color: #140620; font-size: 11px; padding: 3px 8px; border-radius: 4px; font-weight: 900; }.sub-tag { font-size: 12px; color: #a78bfa; margin-top: 4px; }.trading-metrics { display: flex; gap: 25px; align-items: center; flex-wrap: wrap; }.metric-box { background: rgba(19, 6, 32, 0.7); border: 1px solid #4a256d; border-radius: 10px; padding: 10px 18px; text-align: center; }.metric-label { font-size: 10px; color: #a78bfa; font-weight: bold; text-transform: uppercase; letter-spacing: 0.5px; }.metric-val { font-size: 18px; font-weight: bold; color: #ffffff; margin-top: 3px; }.price-display { text-align: right; background: rgba(0,0,0,0.2); padding: 12px 20px; border-radius: 12px; border-right: 4px solid #10b981; }.main-price { font-size: 30px; font-weight: 900; color: #fdb913; font-family: 'Courier New', monospace; }.btc-pair { font-size: 13px; color: #f59e0b; font-weight: bold; margin-top: 2px; }.price-change { font-size: 12px; font-weight: bold; padding: 2px 8px; border-radius: 12px; display: inline-block; margin-top: 5px; }.up { background: rgba(16, 185, 129, 0.2); color: #10b981; border: 1px solid #10b981; }.stats-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; margin-bottom: 25px; }.stat-card { background: #2b1442; border: 1px solid #4a256d; border-radius: 10px; padding: 18px; text-align: center; }.stat-num { font-size: 22px; font-weight: bold; color: white; margin-top: 5px; }h2 { color: #fdb913; margin-top: 20px; border-bottom: 1px solid #4a256d; padding-bottom: 8px; font-size: 15px; font-weight: 700; }.box { background: #2b1442; padding: 20px; border-radius: 12px; margin-bottom: 25px; border-top: 4px solid #8b5cf6; border-left: 1px solid #4a256d; border-right: 1px solid #4a256d; border-bottom: 1px solid #4a256d; box-shadow: 0 4px 15px rgba(0,0,0,0.3); }
.box.deposit { border-top-color: #10b981; }.box.withdraw { border-top-color: #ef4444; }.box.p2p { border-top-color: #3b82f6; }.box.tree { border-top-color: #fdb913; }.form-inline { display: flex; flex-wrap: wrap; gap: 10px; align-items: center; }input, select { background: #140620; color: white; border: 1px solid #5a2a8a; padding: 10px; border-radius: 8px; box-sizing: border-box; min-width: 140px; outline: none; font-size: 13px; }
input:focus, select:focus { border-color: #fdb913; }.btn { color: white; padding: 10px 18px; border: none; border-radius: 8px; cursor: pointer; font-weight: bold; white-space: nowrap; transition: 0.2s; }.btn:hover { opacity: 0.9; transform: translateY(-1px); }.table-wrapper { width: 100%; overflow-x: auto; background: #140620; margin-top: 10px; border-radius: 10px; border: 1px solid #3c1b5d; }table { width: 100%; border-collapse: collapse; min-width: 600px; }th, td { padding: 12px; text-align: left; border-bottom: 1px solid #2b1442; font-size: 13px; }th { background-color: #10041a; color: #fdb913; font-weight: 700; }.logout-btn { background: #ef4444; color: white; text-decoration: none; padding: 6px 14px; border-radius: 6px; font-size: 12px; font-weight: bold; }.ref-box { background: #10041a; border: 1px dashed #fdb913; padding: 12px; border-radius: 8px; color: #fdb913; font-family: monospace; font-size: 13px; display: flex; justify-content: space-between; align-items: center; margin-top: 10px; gap: 10px; flex-wrap: wrap; }
.copy-btn { background: #fdb913; color: #140620; border: none; padding: 6px 14px; font-weight: bold; border-radius: 6px; cursor: pointer; }.msg-alert { padding: 12px; border-radius: 8px; margin-bottom: 20px; font-size: 14px; font-weight: bold; text-align: center; }.msg-success { background: rgba(16, 185, 129, 0.2); color: #10b981; border: 1px solid #10b981; }.msg-error { background: rgba(239, 68, 68, 0.2); color: #ef4444; border: 1px solid #ef4444; }</style></head><body><h1><div class="brand-head">""" + SMALL_COIN_SVG + """<span class="brand-title">WELCOME, {{ user.name if user else 'MEMBER' }}</span></div><div><span style="font-size: 12px; color: #fdb913; border: 1px solid #fdb913; padding: 4px 12px; border-radius: 20px; margin-right: 10px; background: rgba(253,185,19,0.1);">NODE: {{ user.uid if user else '' }}</span>
<a href="/logout" class="logout-btn">LOGOUT</a></div></h1>{% if msg %}<div class="msg-alert msg-{{ msg_type }}">{{ msg }}</div>{% endif %}<div class="trading-hero"><div class="hero-left">""" + BIG_COIN_SVG + """<div><div class="coin-details-title">B4U COIN <span class="pair-badge">B4U / BTC</span></div><div class="sub-tag">⚡ Sovereign Digital Asset • Automated Liquidity Engine</div></div></div><div class="trading-metrics"><div class="metric-box"><div class="metric-label">BITCOIN BENCHMARK</div><div class="metric-val" style="color: #f59e0b;">${{ "{:,.2f}".format(btc_usd) }}</div></div><div class="metric-box"><div class="metric-label">NETWORK LIQUIDITY</div><div class="metric-val">${{ "{:,.2f}".format(total_inv) }}</div></div><div class="metric-box"><div class="metric-label">YOUR HOLDINGS</div><div class="metric-val" style="color: #fdb913;">{{ coin_holdings }} B4U</div></div></div><div class="price-display"><div class="main-price">${{ coin_price }} USD</div><div class="btc-pair">₿ {{ b4u_in_btc }} BTC</div><div class="price-change up">{% if coin_change >= 0 %}+{% endif %}{{ coin_change }}% Growth</div></div></div><div class="stats-grid"><div class="stat-card" style="border-left: 4px solid #fdb913;"><small style="color:#a78bfa;">MY RANK</small><div class="stat-num" style="color:#fdb913;">{{ user.rank if user else 'Tiffany' }}</div></div><div class="stat-card" style="border-left: 4px solid #10b981;"><small style="color:#a78bfa;">TOTAL ACTIVE INVESTMENT</small><div class="stat-num">${{ user.inv if user else 0.0 }}</div></div><div class="stat-card" style="border-left: 4px solid #8b5cf6;"><small style="color:#a78bfa;">PROFIT WALLET</small><div class="stat-num">${{ user.profit_wallet if user else 0.0 }}</div></div><div class="stat-card" style="border-left: 4px solid #3b82f6;"><small style="color:#a78bfa;">TEAM VOLUME</small><div class="stat-num">${{ team_volume }}</div></div></div><div class="box"><h2>🔗 Your Personal Live Referral Link</h2><p style="font-size: 13px; color: #a78bfa; margin: 0;">Share your exact live domain link to register downline members directly under you:</p><div class="ref-box"><span id="refUrl">{{ ref_url }}</span><button onclick="copyRefLink()" class="copy-btn">COPY LINK</button></div></div><div class="box p2p"><h2>💸 P2P Wallet Transfer (Member-to-Member)</h2><form action="/p2p_transfer" method="POST" class="form-inline"><input type="text" name="recipient_uid" placeholder="Recipient Node UID (e.g. B4U1003)" required style="width:230px;"><input type="number" step="0.01" name="amount" placeholder="Amount ($)" required><button type="submit" class="btn" style="background: linear-gradient(135deg, #3b82f6, #1d4ed8);">TRANSFER FROM PROFIT WALLET</button></form></div><div class="box tree"><h2>🌳 Downline Network Tree</h2><div class="table-wrapper"><table><thead><tr><th>Level</th><th>Node UID</th><th>Member Name</th><th>Rank</th><th>Active Investment</th><th>Status</th></tr></thead><tbody>{% for member in downline_tree %}<tr><td><b style="color:#fdb913;">L{{ member.level }}</b></td><td><code>{{ member.uid }}</code></td><td>{{ member.name }}</td><td>{{ member.rank }}</td><td><b style="color:#10b981;">${{ member.inv }}</b></td><td><span style="color:#10b981;">{{ member.status }}</span></td></tr>{% else %}<tr><td colspan="6" style="text-align:center; color:#a78bfa;">No downline team members found yet. Share your link to recruit partners!</td></tr>{% endfor %}</tbody></table></div></div><div class="box deposit"><h2>📥 Deposit Capital</h2><form action="/deposit" method="POST" class="form-inline"><select name="method" required><option value="USDT (TRC20)">USDT (TRC20)</option><option value="Bank Transfer">Bank Transfer</option><option value="EasyPaisa/JazzCash">EasyPaisa / JazzCash</option></select><input type="number" step="0.01" name="amount" placeholder="Amount ($)" required><button type="submit" class="btn" style="background: linear-gradient(135deg, #10b981, #059669);">SUBMIT DEPOSIT PROOF</button></form></div><div class="box withdraw"><h2>📤 Withdraw Funds</h2><form action="/withdraw" method="POST" class="form-inline"><select name="method" required><option value="USDT TRC20">USDT TRC20</option><option value="Bank Account">Bank Account</option><option value="JazzCash/EasyPaisa">JazzCash / EasyPaisa</option></select><input type="text" name="address" placeholder="Wallet Address / Acc Number" required style="width:220px;"><input type="number" step="0.01" name="amount" placeholder="Amount ($)" required><button type="submit" class="btn" style="background: linear-gradient(135deg, #ef4444, #dc2626);">REQUEST WITHDRAWAL</button></form></div><div class="box"><h2>📜 Recent Transaction History</h2><div class="table-wrapper"><table><thead><tr><th>Type</th><th>Details / Address</th><th>Amount</th><th>Status / Date</th></tr></thead><tbody>{% for p2p in p2p_history %}<tr><td><span style="color:#3b82f6; font-weight:bold;">P2P TRANSFER</span></td><td>From <code>{{ p2p.sender }}</code> ➔ To <code>{{ p2p.recipient }}</code></td><td><b>${{ p2p.amount }}</b></td><td><i>{{ p2p.created_at }}</i></td></tr>{% endfor %}{% for dep in deposits %}<tr><td><span style="color:#10b981; font-weight:bold;">DEPOSIT</span></td><td>{{ dep.method }}</td><td><b>${{ dep.amount }}</b></td><td><i>{{ dep.status }}</i></td></tr>{% endfor %}{% for wit in withdrawals %}<tr><td><span style="color:#ef4444; font-weight:bold;">WITHDRAWAL</span></td><td><code>{{ wit.method }} - {{ wit.address }}</code></td><td><b>${{ wit.amount }}</b></td><td><i>{{ wit.status }}</i></td></tr>{% endfor %}</tbody></table></div></div><script>function copyRefLink() {var refText = document.getElementById("refUrl").innerText;navigator.clipboard.writeText(refText);alert("Referral Link copied to clipboard!");}</script></body></html>"""
...
def user_register_page():
    ...
    with get_db() as conn:
        conn.execute(
            "INSERT INTO users (uid, name, password, rank, inv, profit_wallet, status, referrer, created_at) VALUES (?, ?, ?, 'Tiffany', 0.0, 0.0, 'Active', ?, ?)",
...
def user_p2p_transfer():
    ...
    with get_db() as conn:
        ...
        conn.execute("UPDATE users SET profit_wallet = round(profit_wallet + ?, 2) WHERE uid = ?", (amount, recipient_uid))
        conn.execute(
