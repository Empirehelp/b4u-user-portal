...
def user_dashboard():
    ...
    if ref_code and not session.get('user_uid'):
        return redirect(url_for('user_bp.user_register_page', ref=ref_code))
    ...
    try:
        ...
        user = cur.fetchone()
        cur.execute(
    "SELECT * FROM deposits WHERE uid = %s ORDER BY id DESC", (uid,))
        deposits = cur.fetchall()
        cur.execute(
    "SELECT * FROM withdrawals WHERE uid = %s ORDER BY id DESC", (uid,))
        withdrawals = cur.fetchall()
        cur.execute(
    "SELECT * FROM p2p_transfers WHERE sender = %s OR recipient = %s ORDER BY id DESC",
    (uid,
     uid))
...
def spin_wheel():
    ...
    try:
     ...
     "message": "You have already spun the wheel!"})
        ...
        cur.execute(
    "UPDATE users SET inv = round(inv + %s, 2), wheel_spun = TRUE WHERE uid = %s",
    (prize,
     uid))
        conn.commit()
     finally:
...
@user_bp.route('/p2p_transfer', methods=['POST'])
...
def user_deposit():
    ...
    if amount > 0:
        ...
        try:
            cur.execute("""
                INSERT INTO deposits (uid, amount, method, status, created_at) 
                VALUES (%s, %s, %s, '⏳ Pending Verification', %s)
